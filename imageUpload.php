<?php

	session_start();
	$error = '';

	$img_name = rand(1, 1000);

	$img_address_orig = "F:\BLOCKCHAIN/".$img_name."icon_orig.jpg";
	$img_address = "F:\BLOCKCHAIN/".$img_name."icon.jpg";


	$success = 0;

	if($_FILES["icon_img"]["error"] > 0) {
		echo "Error: " . $_FILES["icon_img"]["error"] . "<br/>";
	}
	else {
		if(move_uploaded_file($_FILES["icon_img"]["tmp_name"], $img_address)) {
			copy($img_address, $img_address_orig);
			echo "Image uploaded";
			$success = 1;
		}
	}

	if($success) {

		$result = shell_exec('python script.py ' . $img_address);

		$_SESSION["image_path"] = $img_address;
		$_SESSION["image_path_orig"] = $img_address_orig;
		
		header('Location: upload.php');
	}
	else {
		echo "Image file couldn't be uploaded. File size invalid.";
	}
	

?>